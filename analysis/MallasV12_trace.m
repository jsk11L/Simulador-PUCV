% PROGRAMA DE SIMULACI�N DE CARRERAS
% JORGE MENDOZA BAEZA
% FECHA 03 JULIO 2018
% 20 JULIO 2023
...
fprintf('[TRACE] Archivo: Civilelectrica93.xlsx\n');
fprintf('[TRACE] Hoja de malla: Malla\n');
fprintf('[TRACE] Hoja de programacion: ProgramacionB\n');
fprintf('[TRACE] Asignaturas cargadas: %d\n', size(ASIGNATURAS,1));
fprintf('[TRACE] Filas programacion cargadas: %d\n', size(PROGRAMACION,1));
  
% VARIABLES DE SIMULACI�N
NE=2;      %Numero de estudiantes
NCSmax=21;  % N�mero de cr�ditos semestrales m�ximos
TAmin=12.3; % Tasa de avance m�nima
NapTAmin=10; % Semestre aplicaci�n de tasa de avance 
Opor=6;     % Numero maximo de oportunidades para reprobar un ramo

% MODELO DE CALIFICACIONES
VMap1234=0.48;   % Valor medio de aprobaci�n  r=VMap1+Delta1.*randn(l,1) por semestre
Delta1234=0.2;  %Variacion de la aprobaci�n por semestre
VMap5678=0.55;   % Valor medio de aprobaci�n  r=VMap1+Delta1.*randn(l,1) por semestre
Delta5678=0.2;  %Variacion de la aprobaci�n por semestre
VMapM=0.65;   % Valor medio de aprobaci�n  r=VMap1+Delta1.*randn(l,1) por semestre
DeltaM=0.25;  %Variacion de la aprobaci�n por semestre

%INICIALIZACI�N DE VARIABLES
Resumen=[];         
[CA,T]=size(ASIGNATURAS);           % Cantidad de asignaturas
Reprobadas=ASIGNATURAS(:,2);        % Guardar el nombre de las  asignaturas para reprobaci�n
Reprobadas(1,2+NE)=0;                  % Ajuste para crear matriz

% CICLO PARA EL N�MERO DE ESTUDIANTES

for k=1:NE
    fprintf('\n[TRACE] ===== Alumno %d/%d =====\n', k, NE);

    % EVOLUCI�N DEL PRIMER SEMESTRE
    ...
    fprintf('[TRACE] Semestre %d: %d asignaturas detectadas en malla.\n', Sem, l);

    r=abs(VMap1234+Delta1234.*randn(l,1));    %crear vector aleatorio para comprobar aprobaci�n del primer semestre
    Ap=(r>=ASIGNATURAS(a,4));           % Revisar cuales asignaturas aprob�
    ...
    HA(:,4)=ASIGNATURAS(a,3);           % Guaradr el n�mero de cr�ditos
    NAp=sum(Ap);                        % Cuantas asignaturas est�n aprobadas
    ...
    if (Sem>=NapTAmin)&&(TA(Sem)<TAmin) % SEmestre de aplicaci�n de la tasa de avance
        Estado=1;
    end
    ...
    % EVOLUCI�N DE LOS SEMESTRES POR ALUMNO
    while (NAp<CA)&&(Estado~=1)
        if Sem>1
            [f,c]=find(HA(:,3)==2);      % Determinar los cursos tomados el semestre
            l=length(f);                 % Saber cuantos cursos tom�
                Inscritas=0;             %Inicializar variable
                for i=1:l
                    Inscritas(i)=find(ASIGNATURAS(:,2)==HA(f(i),2));     % revisar cuales de las asignaturas fueron tomadas
                end
                PAprI=ASIGNATURAS(Inscritas,4);       % Determinar los porcentajes de aprobaci�n de las asignaturas inscritas
                if (Sem==2||Sem==3||Sem==4)
                    r=abs(VMap1234+Delta1234.*randn(l,1));              %crear vector aleatorio para comprobar aprobaci�n del semestre
                ...
                %r=rand(l,1);
                Ap=(r>=PAprI);                        % Revisar cuales asignaturas aprob�
                HA(f,3)=Ap;                           %Cambiar el estado de las asignaturas tomadas a aprobadas o reprobadas
                HA(f,4)=ASIGNATURAS(Inscritas,3);     % Guardar el numero de cr�ditos
        ...
        fprintf('[TRACE] Semestre %d: oferta inicial %d asignaturas.\n', Sem, length(ProgD));
        l=length(ProgD);                % N�mero de asignaturas posibles de tomar
        i=1;
        while i<=l                      %procedimiento para aliminar del vector de programaci�n las asignaturas aprobadas
        ...
        fprintf('[TRACE] Semestre %d: oferta depurada %d asignaturas.\n', Sem, length(ProgD));
           
        
        l=length(ProgD);
        NCS=0;              % Numero de creditos semestrales
        for i=1:l                       % Ciclo para revisar las asignaturas que tomar� 
            ...
            if auxA~=0
                ...
                if (auxPr==auxNAPr)||(auxNAPr==0);   % si cumple los requisitos inscribo la asignatura en el historial o no tiene prerequisitos
                    if (NCS+ASIGNATURAS(f,3))<=NCSmax
                        ...
                        fprintf('[TRACE] Sem %d: inscrita asignatura %d (cred %d, NCS=%d).\n', Sem, auxA, ASIGNATURAS(f,3), NCS);
                    end
                end
            end

        end
        % se revisar� su pudo tomar algo
...
[x,y]=find(Resumen(:,5)<15);    % Encontrar los que terminan al d�a
Aldia=0;
for i=1:length(x)
    if (Resumen(x(i),2)==2)
        Aldia=Aldia+1;
...
% Calculo de los porcentajes de retenci�n
aux=0;
...
Resumen;
fprintf('AN�LISIS DE LOS RESULTADOS \n')
fprintf(' Promedio: %3.2f %3.2f %3.2f Desviaci�n: %3.2f %3.2f %3.2f\n',VMap1234,VMap5678,VMapM, Delta1234,Delta5678,DeltaM)
% fprintf(' Promedio: %3.2f Desviaci�n: %3.2f\n',VMap5678, Delta5678)
% fprintf(' Promedio: %3.2f Desviaci�n: %3.2f\n',VMapM, DeltaM)
...
fprintf(' Alumnos Eliminados al 1er a�o: %3.2f %% de Retencion (%3.2f)\n', (NE-EEpA)*100/NE, EEpA)
fprintf(' Alumnos Eliminados al 3er a�o: %3.2f %% de Retencion (%3.2f)\n', (NE-EEtA)*100/NE, EEtA)
